...
Router.prototype.handleRequest = function(req, callback) {
  ...
  options.include = (Array.isArray( include ) ? include : include ? [ include ] : []).map( function(modelName) {
    var factory = self.findDAOFactory(modelName);
    if( ! factory ) {
      self.handleError('Unknown include: ' + modelName, callback)
    }
    return factory;
  });
  ...
  switch(match.length) {
    case 0: // requested path: /api
      ...
      if ((req.method === 'GET') && (req.path === this.options.endpoint)) {
        handleIndex.call(this, options, callback)
      }else{
    ...
    case 1: // requested path: /api/dao_factory
      ...
      if (this.isAllowed(modelName)){
        switch(req.method) {
          case "GET":
            handleResourceIndex.call(this, modelName, options, callback)
            break
          case "HEAD":
            handleResourceDescribe.call(this, modelName, options, callback)
            break
          case "POST":
            handleResourceCreate.call(this, modelName, options, req.body, callback)
            break
          default:
    ...
    case 2: // requested path: /api/dao_factory/1
      ...
      if (parseInt(identifier) === NaN || !this.isAllowed(modelName)){
        ...
        switch(req.method) {
          case 'GET':
            handleResourceShow.call(this, modelName, options, identifier, callback)
            break
          case 'DELETE':
            handleResourceDelete.call(this, modelName, options, identifier, callback)
            break
          case 'PUT':
            handleResourceUpdate.call(this, modelName, options, identifier, req.body, callback)
            break
          default:
            this.handleError('Method not available for this pattern.', callback)
            break
    ...
    case 3: // requested path: /api/dao_factory/1/associated_dao_factory
      ...
      if (parseInt(identifier) === NaN || !this.isAllowed(modelName) || !this.isAllowed(associatedModelName)){
        ...
        switch(req.method) {
          case 'GET':
            handleResourceIndexAssociation.call(this, modelName, options, identifier, associatedModelName, callback)
            break
          case 'DELETE':
            // Search for a 1:1 / N:1 association. If it exists, we can dereference the target.
            // Otherwise we will raise an error.
            this.findSingleAssociatedModel(modelName, options, identifier, associatedModelName, function(err, associatedModel) {
              if (!!associatedModel) {
                handleResourceDeleteAssociation.call(this, modelName, options, identifier, associatedModelName, associatedModel.id, callback)
              } else {
                this.handleError('Method not available for this pattern.', callback)
              }
            }.bind(this))
    ...
    case 4: // requested path: /api/dao_factory/1/associated_dao_factory/1
      ...
      if (parseInt(identifier) === NaN || associatedIdentifier === NaN || !this.isAllowed(modelName) || !this.isAllowed(associatedModelName)){
        ...
        switch(req.method) {
          case 'DELETE':
            handleResourceDeleteAssociation.call(this, modelName, options, identifier, associatedModelName, associatedIdentifier, callback)
            break
          // Add more handlers for others methods
          default:
            this.handleError('Method not available for this pattern.', callback)
...
var handleIndex = function(options, callback) {
...
var handleResourceIndex = function(modelName, options, callback) {
  var daoFactory = this.findDAOFactory(modelName)

  if (!!daoFactory) {
    daoFactory
      .findAll({ include: options.include })
...
var handleResourceDescribe = function(modelName, options, callback) {
  var daoFactory = this.findDAOFactory(modelName)
...
var handleResourceShow = function(modelName, options, identifier, callback) {
  var daoFactory = this.findDAOFactory(modelName)

  if (!!daoFactory) {
    daoFactory
      .find({ where: { id: identifier }, include: options.include })
...
var handleResourceCreate = function(modelName, options, attributes, callback) {
  var daoFactory = this.findDAOFactory(modelName)
...
var handleResourceUpdate = function(modelName, options, identifier, attributes, callback) {
  var daoFactory = this.findDAOFactory(modelName)
...
var handleResourceDelete = function(modelName, options, identifier, callback) {
  var daoFactory = this.findDAOFactory(modelName)
...
var handleResourceIndexAssociation = function(modelName, options, identifier, associatedModelName, callback){
  var daoFactory  = this.findDAOFactory(modelName)
    , association = !!daoFactory ? this.findAssociation(daoFactory, associatedModelName) : null
...
var handleResourceDeleteAssociation = function(modelName, options, identifier, associatedModelName, associatedIdentifier, callback){
  var daoFactory           = this.findDAOFactory(modelName)
    , association          = !!daoFactory ? this.findAssociation(daoFactory, associatedModelName) : null
